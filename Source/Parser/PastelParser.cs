...
namespace Pastel.Parser
{
    internal class PastelParser
    {
        ...
        {
            string lang = LanguageUtil.GetFileExtension(context.Language).Substring(1);
            ...
            while (tokens.HasMore)
            {
                switch (tokens.PeekValue())
                {
                    ...
                    case "@":
                        ...
                        ICompilationEntity[] inlinedEntities = ParseTopLevelInlineImport(atToken, tokens);
        ...
        private ICompilationEntity[] ParseTopLevelInlineImport(Token atToken, TokenStream tokens)
        {
            ...
            switch (functionName)
            {
                ...
                case "importIfFalse":
                    ...
                    tokens.PopExpected(")");
                    tokens.PopExpected(";");
                    object value = GetConstant(constantValue, false);
            ...
            if (sourceFile != null)
            {
                string code = importCodeLoader.LoadCode(atToken, sourceFile);
        ...
        {
            string code = importCodeLoader.LoadCode(importToken, path);
        ...
        public VariableDeclaration ParseConstDefinition(TokenStream tokens)
        {
            ...
            VariableDeclaration assignment = ParseAssignmentWithNewFirstToken(constToken, tokens);
        ...
        public ClassDefinition ParseClassDefinition(TokenStream tokens)
        {
            ...
            ClassDefinition cd = new ClassDefinition(context, classToken, nameToken);
            ...
            while (!tokens.PopIfPresent("}"))
            {
                ...
                if (next == "constructor")
                {
                    ...
                    cd.Constructor = new ConstructorDefinition(context, constructorToken, argTypes, argNames, cd);
                    currentCodeOwner = cd.Constructor;
                    cd.Constructor.Code = ParseCodeBlock(tokens, true).ToArray();
                }
                else
                {
                    ...
                    if (isMethod)
                    {
                        ...
                        currentCodeOwner = fd;
                        List<Executable> code = ParseCodeBlock(tokens, true);
                    ...
                    {
                        FieldDefinition fd = new FieldDefinition(context, memberType, memberName, cd);
                        currentCodeOwner = fd;
                        Expression initialValue = null;
                        if (tokens.PopIfPresent("="))
                        {
                            initialValue = ParseExpression(tokens);
                        }
                        else
                        {
        ...
        public EnumDefinition ParseEnumDefinition(TokenStream tokens)
        {
            ...
            EnumDefinition enumDef = new EnumDefinition(enumToken, nameToken, context);
            currentCodeOwner = enumDef;
        ...
        public StructDefinition ParseStructDefinition(TokenStream tokens)
        {
            ...
            return new StructDefinition(structToken, nameToken, structFieldTypes, structFieldNames, parentName, context);
        }
        ...
        public Executable ParseExecutable(TokenStream tokens, bool isForLoop)
        {
            ...
            if (assignmentType != null && tokens.HasMore && IsValidName(tokens.PeekValue()))
            {
                ...
                if (tokens.PopIfPresent(";"))
                {
                    return new VariableDeclaration(assignmentType, variableName, null, null, context);
                }
                ...
                return new VariableDeclaration(assignmentType, variableName, equalsToken, assignmentValue, context);
            }
                ...
                while (thereAreCases)
                {
                    switch (tokens.PeekValue())
                    {
                        case "case":
                            ...
                            Expression caseExpression = ParseExpression(tokens);
                            tokens.PopExpected(":");
        ...
        private Expression ParseEntity(TokenStream tokens)
        {
            if (tokens.IsNext("new"))
            {
                ...
                Expression constructorReference = new ConstructorReference(newToken, typeToConstruct, currentCodeOwner);
        ...
        private Expression ParseEntityRoot(TokenStream tokens)
        {
            ...
            switch (next)
            {
                case "true":
                case "false":
                    return new InlineConstant(PType.BOOL, tokens.Pop(), next == "true", currentCodeOwner);
                case "null":
                    return new InlineConstant(PType.NULL, tokens.Pop(), null, currentCodeOwner);
                case ".":
                    ...
                    {
                        return new InlineConstant(PType.DOUBLE, dotToken, dblValue, currentCodeOwner);
                    }
            ...
            {
                case '\'':
                    return new InlineConstant(PType.CHAR, tokens.Pop(), CodeUtil.ConvertStringTokenToValue(next), currentCodeOwner);
                case '"':
                    return new InlineConstant(PType.STRING, tokens.Pop(), CodeUtil.ConvertStringTokenToValue(next), currentCodeOwner);
                case '@':
                    ...
                    return new CompileTimeFunctionReference(atToken, compileTimeFunction, currentCodeOwner);
            }
            ...
            if (firstChar >= '0' && firstChar <= '9')
            {
                ...
                if (tokens.IsNext("."))
                {
                    ...
                    {
                        return new InlineConstant(PType.DOUBLE, numToken, dblValue, currentCodeOwner);
                    }
                ...
                else
                {
                    ...
                    return new InlineConstant(PType.INT, numToken, numValue, currentCodeOwner);
                }
            }

            if (tokens.IsNext("this"))
            {
                return new ThisExpression(tokens.Pop(), currentCodeOwner);
            }

            if (IsValidName(tokens.PeekValue()))
            {
                return new Variable(tokens.Pop(), currentCodeOwner);
            }
